...
`peerd [interval]` (default 300s) is the companion daemon: it discovers peers (the `gossipRootIps` API + harvesting a running node's logs) and probes each for **live-block serving** (`send_abci:false`, cheap and not rate-limited — it never probes the rate-limited abci_state), writing a ranked pool to `<data-dir>/peers.json` that `gateway` reads and refreshes every 30s. It's a subcommand of the same `hypersync` binary (no more Python/curl-in-bash), but deliberately still runs as its **own OS process**, not a background task inside the gateway — a probing storm across 100+ candidates or a hung `docker logs` call never touches the process actively serving a node, and either side can be restarted independently. Config is via env vars for a drop-in swap from the old `peerd.sh`: `HYPERSYNC_DATA` (data dir, default `./data`), `HL_NODE` (node container to harvest peers from, default `hyperliquid-node-1`), `HL_SELF_IP` (optional — exclude the node's own public IP from candidates, since it's a legitimate routable address but not a peer to dial).
