...
A node points `root_node_ips` at the gateway and connects to nothing else; the gateway fronts a pool of real peers (kept fresh + ranked by `peerd`, which discovers via `gossipRootIps` + the node's own `tcp_lz4_stats/<date>` files (read from the hl-data volume) and probes live-serving — never the rate-limited state).
...
- **Cache cold-start (`--cache`).** The gateway captures the full §7 bootstrap **verbatim** from a fast peer (speed-gate >5 MB/s; stop when the byte-rate collapses to the live trickle for ~12 s = bulk end) and holds it in memory, with a persisted cache file (`bootstrap.cache` beside the peer file, or `/tmp/hypersync-bootstrap.cache` if that directory is read-only; override with `HYPERSYNC_BOOT_CACHE`). On a node's `send_abci:true` it **replays fresh cached bytes** (<=30 minutes old) — the node cold-starts from cache with **no peer state fetch**. The gateway refreshes cache every 10 minutes and retries failed refreshes after 2 minutes; stale cache is ignored and the node transparently bootstraps from a real peer, while the gateway can tee that successful fallback stream back into cache.
